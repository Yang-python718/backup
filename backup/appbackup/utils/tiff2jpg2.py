...
class tran():
    ...
    def stretchImg(self,imgPath, resultPath, lower_percent=0.6, higher_percent=99.4):
        ...
        a = 0
        b = 255
        c = np.nanpercentile(data[:, :], lower_percent)
        d = np.nanpercentile(data[:, :], higher_percent)
        t = a + (np.array(data[:, :]) - np.array(c)) * (b-a) / (np.array(d) - np.array(c))
        t[t < a] = a
        t[t > b] = b
        self.t=t
        out[:, :] = t
        # out[i,:,:]=t
        self.out=out
        Image.fromarray(np.uint8(out)).convert('RGB').save(resultPath, format="png")
